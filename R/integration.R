integration <-
function(data,reset=FALSE,reset.criteria=c("samples","time","value"),vreset,units,data.name)
   {
   call <- match.call()
   if(missing(data)) 
   ...
   object <- iemg(ivalues,match.call(),resetpoints,data$samplingrate,units,data.name=data.name)
   return(object)
   }
