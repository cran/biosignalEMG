eaemg <-
function(data,runs,what,timenormalization=c("min","mean","median","max"),scalem=1,empirical=TRUE,level=0.90)
   {
   call <- match.call()
   if(missing(data)) 
   ...
   if (is.null(runs$lengths) || is.null(runs$values) || length(runs$lengths) != length(runs$values)) 
        stop("invalid 'runs' structure")
   timenormalization <- match.arg(timenormalization)
   ftn<-get(timenormalization)
   if(!isTRUE(empirical)) empirical<-FALSE
