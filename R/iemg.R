iemg <-
function(data,calliemg,resetpoints,samplingrate=0,units="",data.name="")
   {
   call <- match.call()
   if(missing(data)) 
   ...
   object<-list(values=data,call=calliemg,reset.points=resetpoints,units=units,samplingrate=samplingrate,data.name=data.name)
   class(object)<-"iemg"
   return(object)   
   }
