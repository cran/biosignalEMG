synthetic.emg <-
function(n.length.out=10000,on.sd=1,on.duration.mean=350,on.duration.sd=10,off.sd=0.05,off.duration.mean=300,off.duration.sd=20,on.mode.pos=0.75,shape.factor=0.5,samplingrate=0,units="",data.name="Synthetic EMG")
   {
   call <- match.call()
   ...
   while(i<n.length.out)
      ...
      semg<-c(semg,rnorm(n.off,0,off.sd),c(seq(off.sd/on.sd,1,length.out=round(on.mode.pos*n.on)),seq(1,off.sd/on.sd,length.out=n.on-round(on.mode.pos*n.on)))^(shape.factor)*rnorm(n.on,0,on.sd))
      i <- i + n.off + n.on
      }
   b <- head(b,n.length.out)
   semg <- head(semg,n.length.out)
   object<-list(values=semg,units=units,samplingrate=samplingrate,data.name=data.name,on.off=b)
   class(object)<-"emg"
   return(object)
   }
